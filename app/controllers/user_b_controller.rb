class UserBController < ApplicationController
  ...
  def create
  	if params[:login][:userid] == "1222000" && params[:login][:password] == "password"
  	  user = UserB.new(login_params)
      user.save
  ...
  def login_comp
    ...
    if @timesa <= 3 && @timesa >= -3
      ...
      UserB.order("created_at desc").limit(1).update_all(:onetime => @onetime)
    else
      redirect_to :action => "login_failure"
    end
  end
